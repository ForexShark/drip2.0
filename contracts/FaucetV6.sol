...
contract FaucetV6 is OwnableUpgradeable {
    ...
    // Animal Farm liquidator
    IFeeManagerDogs public feeManagerDogs;
    bool public isLiquifierEnabled;

    // Drip TWAP
    IDripTWAP public dripTWAP;
    bool public isTWAPEnabled;

    // V6 - whitelist for alternative deposits
    mapping(address => bool) private deposit_whitelist;
    ...
    function updateHoldRequirements(
        uint256[] memory _newRefBalances
    ) public onlyOwner {
    ...
    function authorizeDepositor(address addr) external onlyOwner {
        deposit_whitelist[addr] = true;
    }

    function deauthorizeDepositor(address addr) external onlyOwner {
        deposit_whitelist[addr] = false;
    }

    function isDepositor(address addr) public view returns (bool) {
        return deposit_whitelist[addr];
    }

    modifier onlyDepositor() {
        require(isDepositor(_msgSender()), "Not authorized");
        _;
    }
    ...
    /** @dev Allow authorized contracts to deposit on users behalf.
     * _amount is gross amount deposited, _net_amount is the amount after taxes have been deducted
     */
    function depositFor(
        address _addr,
        address _upline,
        ...
        require(_amount >= _net_amount, "Invalid arguments");
        require(_amount >= minimumAmount, "Minimum deposit");
        ...
        if (claimsAvailable(_addr) > _amount / 100) {
            ...
            _net_amount += taxedDivs;
            taxedDivs = taxedDivs / 2;
        }

        //Transfer DRIP to the contract
        // uint256 to_vault = _amount - _net_amount;
        require(
        ...
        // if (to_vault > 0) {
        //     require(dripToken.transfer(dripVaultAddress, to_vault));
        // }

        _deposit(_addr, _net_amount);

        _refPayout(_addr, _net_amount + taxedDivs, ref_bonus);
    ...
    function creditsAndDebits(
        address _addr
    ) public view returns (uint256 _credits, uint256 _debits) {
        User memory _user = users[_addr];
        Airdrop memory _000airdrop = airdrops[_addr];

        _credits = _000airdrop.airdrops + _user.rolls + _user.deposits;
        ...
        )
    {
        ...
        max_payout = maxPayoutOf(_addr).min(max_payout_cap);
    ...
    /**
     * @dev Internal airdrop implementation. Assumes _to has been validated
     */
    function _airdrop(address _to, address _from, uint256 amount) internal {
        (uint256 gross_payout, , , ) = payoutOf(_to);

        users[_to].accumulatedDiv = gross_payout;

        //Fund to deposits (not a transfer)
        users[_to].deposits += amount;
        users[_to].deposit_time = block.timestamp;

        //User stats
        airdrops[_to].airdrops_received += amount;

        //Let em know!
        emit NewAirdrop(_from, _to, amount, block.timestamp);
        emit NewDeposit(_to, amount);
    }
    ...
    function airdrop(address _to, uint256 _amount) external {
        ...
        _airdrop(_to, _addr, _realizedAmount);
    ...
    function batchAirdropEqual(
        ...
        require(_destinations.length > 0, "Destination list is empty");
        ...
        uint256 perWallet = _realizedAmount / _destinations.length;
        uint256 dust = _realizedAmount % _destinations.length;

        // To passes so that the transfer is validated ahead of the batch airdrop
        for (uint256 i; i < _destinations.length; i++) {
            // Validate destination
            require(
                users[_destinations[i]].upline != address(0),
                "invalid airdrop destination"
            );
            _airdrop(_destinations[i], _addr, perWallet + dust);
            dust = 0;
        }
        //User stats
        airdrops[_addr].airdrops += _realizedAmount;
        ...
        //Keep track of overall stats
        total_airdrops += _realizedAmount;
        total_txs += _destinations.length; // TODO Check this

        triggerTWAPUpdate();
    }

    // Animal Farm liqufication
    function triggerLiquification() internal {
        if (isLiquifierEnabled && address(feeManagerDogs) != address(0)) {
            try feeManagerDogs.liquefyDogs() {} catch {}
        }
    }

    function toggleFeeManagerDogsLiquefaction(
        bool _isEnabled
    ) external onlyOwner {
        isLiquifierEnabled = _isEnabled;
    }

    function setFeeManagerDogs(address _feeManagerDogs) external onlyOwner {
        feeManagerDogs = IFeeManagerDogs(_feeManagerDogs);
    }

    // Drip TWAP
    function triggerTWAPUpdate() internal {
        if (isTWAPEnabled && address(dripTWAP) != address(0)) {
            try dripTWAP.update() {} catch {}
        }
    }

    function toggleDripTWAPEnabled(bool _isEnabled) external onlyOwner {
        isTWAPEnabled = _isEnabled;
    }

    function setDripTWAP(address _dripTWAP) external onlyOwner {
        dripTWAP = IDripTWAP(_dripTWAP);
    }
}
